...
2026-09-15に本番反映したLIFT JOB応募連携について、応募者向けメール・SMSを発生させずに、Lark通知WebhookとAnyCross/Base連携Webhookが修正後の項目を受理することを確認する。
...
- 本番コード・環境変数は変更しないため、システムのロールバックは不要。追加した回帰アサーションは当該テスト差分を戻せば元に戻る。
...
- `ridejob-form` の実値を `ridejob-entry` の本番通知／Base環境変数へ設定し、両プロジェクトへ共通の `HEALTH_CHECK_TOKEN` を設定する作業は、Vercel再デプロイとGitHub Actionsの実行枠消費を伴うためユーザー承認待ち。
