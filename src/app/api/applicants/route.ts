...
export async function POST(request: NextRequest) {
  try {
    ...
    // 副作用（Lark通知・Base保存・メール・SMS・CAPI）の失敗を1箇所に集める（src/lib/side-effects.ts）。
    // throw だけでなく **HTTPエラーや Lark の非0コードも** markFailed で入れる。入れないと、
    // 最後のサマリ行が「失敗0件」と嘘をつく。
    const { failures: taskFailures, markFailed, trackTask } = createTaskTracker('[applicants]');
    ...
    if (!sendBaseOnly) {
      ...
      // 例外を記録しないと、fetch の throw を allSettled が握りつぶし **ログが1行も出ない**。
      // クーパン専用ルートでは 2026-09-10 に本番の Lark 通知が無言で止まっていた（PR #79）。
      // 同じ形がこのルートにも残っていたので、各タスクは trackTask でくるみ、例外を必ず記録する。
