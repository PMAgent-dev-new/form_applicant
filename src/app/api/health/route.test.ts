...
function stubFetch({
  ...
  larkAuth?: 'ok' | 'bad_credentials' | 'http_500' | 'timeout';
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes('tenant_access_token')) {
        if (larkAuth === 'timeout') throw Object.assign(new Error('timed out'), { name: 'TimeoutError' });
        if (larkAuth === 'http_500') return new Response('{}', { status: 500 });
        if (larkAuth === 'bad_credentials') {
