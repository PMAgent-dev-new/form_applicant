 ...
 *     **求人票側にはこの内訳が無いため、求人票の是正も別途必要。**
 *
 *     ⚠️ 超過分の条項は「**固定残業時間を超える**」と書き、時間数の数字に依存させない。
 *     LPに載せた勤務条件（実働8時間・完全週休2日・土日祝休み・年末年始休暇）から
 *     月平均所定労働時間を約160〜163時間として計算すると、75,150円は
 *     **約40時間分**に一致し、回答の「約35時間分」と数理的に合わない。
 *     35時間分なら法定を上回る単価で支給していることになり違法ではないが、
 *     実際が40時間分だった場合に「35時間を超える分は別途支給」と書くと
 *     35〜40時間帯の残業代を過大に約束することになる。
 *     どちらでも真になる書き方にして、時間数の確定は求人票の是正時に行う。
  ...
  rows: [
    {
      label: '給与',
      value:
        '月給 320,000円 〜\n（基本給 244,850円 ＋ 固定残業代 75,150円／時間外労働 約35時間分）\n※固定残業時間を超える時間外労働、および休日・深夜労働分は別途支給します\n入社後3ヶ月間の獲得件数に応じて、最大100万円の一時金（条件あり）',
    },
