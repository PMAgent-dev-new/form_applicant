...
export default function TruckApplicationComplete() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.dataLayer) {
      window.dataLayer.push({
        'event': 'form_complete',
        'form_name': 'ridejob_truck_application'
      });
    }
  }, []);
  ...
  // 求人情報を取得（トラックドライバー: categoryId=8）
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const response = await fetch(apiPath('/api/jobs/random?categoryId=8&count=3'));
        if (!response.ok) {
      ...
      <main className="flex flex-1 flex-col pb-12">
        ...
        <section className="mt-12">
          ...
          <div className="px-4">
            ...
            <p className="mt-4 text-lg leading-relaxed text-gray-900">
              日本全国の移動と物流を支えることを目指して、ドライバー業界に特化した転職エージェント事業を行っています。
            </p>
          </div>
        ...
        {!isLoadingJobs && jobs.length > 0 && (
          <section className="mt-12 px-4">
            ...
            <div className="px-2 text-sm leading-relaxed text-gray-700">
              <p className="mt-6">
                {new Date().toLocaleDateString('ja-JP', { month: 'long', day: 'numeric' })}現在、トラック求人への問合せが増えています。
              </p>
