 ...
 *
 * 原則:
 *  - **応募が所定の場所（Lark Base）と所定の経路（Lark 通知）の両方に揃わなかったときに書く。**
 *    内訳は3つ: (a) Base への保存が失敗した (b) 通知が落ちた (c) Base か通知の設定が欠けている。
 *    正常時は1行も増えないので、このテーブルに行があること自体が異常で、監視の対象になる。
 *    （当初は (a) だけを想定していたが、(b)(c) でも応募が人に届かないので同じ受け皿に入れる。
 *    2026-09-24 の反証レビューで、コメントと実装がずれていた点を実装側に合わせて訂正）
